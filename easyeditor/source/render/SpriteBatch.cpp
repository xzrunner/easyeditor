...
// 		m_node[5] = hh;
	...
	const float hw = symbol.GetSize().Width() * sprite->GetScale().x * 0.5f,
		hh = symbol.GetSize().Height() * sprite->GetScale().y * 0.5f;
	float x0 = -hw;
	float y0 = -hh;
	float x1 = -hw;
	float y1 = hh;
...
// void SpriteBatch::draw(const std::vector<Sprite*>& sprites) const
// {
...
// 		}
// 
// 		//		const ImageSymbol* curr = &dynamic_cast<ImageSymbol&>(const_cast<Symbol&>(sprite->getSymbol()));
