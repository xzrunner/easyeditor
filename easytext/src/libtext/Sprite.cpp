...
void Sprite::Load(const Json::Value& val, const std::string& dir)
{
	...
	m_text	 = text_val["text"].asString().c_str();
...
void Sprite::Store(Json::Value& val, const std::string& dir) const
{
	...
	text_val["font_color"]		= gum::color2str(m_tb.font_color, s2::RGBA).c_str();

	text_val["edge"]			= m_tb.has_edge;
	text_val["edge_size"]		= m_tb.edge_size;
	text_val["edge_color"]		= gum::color2str(m_tb.edge_color, s2::RGBA).c_str();
