...
namespace opengl
{
	...
	ParticleSystem::ParticleSystem(d2d::Image * sprite, unsigned int buffer) : pStart(0), pLast(0), pEnd(0), active(true), emissionRate(0),
...
// 			this->sprite->release();
	...
	void ParticleSystem::setSprite(d2d::Image * image)
	{
// 		if (sprite != 0)
// 			sprite->release();
...
//		sprite->retain();
	...
	void ParticleSystem::draw(float x, float y, float angle, float sx, float sy, float ox, float oy, float kx, float ky) const
	{
		...
		while (p != pLast)
		{
			...
			t.setTransformation(p->position[0], p->position[1], p->rotation, p->size, p->size, 
				offsetX, offsetY, 0.0f, 0.0f);
