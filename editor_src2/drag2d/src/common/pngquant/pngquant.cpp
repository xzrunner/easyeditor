...
static const struct {const char *old; char *_new;} obsolete_options[] = {
    {"-fs","--floyd"},
    {"-nofs", "--ordered"},
    {"-floyd", "--floyd"},
    {"-nofloyd", "--ordered"},
    ...
    for(int argn=1; argn < argc; argn++) {
        ...
        for(int i=0; i < sizeof(obsolete_options)/sizeof(obsolete_options[0]); i++) {
            if (0 == strcmp(obsolete_options[i].old, argv[argn])) {
                fprintf(stderr, "  warning: option '%s' has been replaced with '%s'.\n", obsolete_options[i].old, obsolete_options[i]._new);
                argv[argn] = obsolete_options[i]._new;
            }
        }
    }
}
...
static float remap_to_palette(read_info *input_image, write_info *output_image, colormap *const map, const float min_opaque_val)
{
    ...
    const int transparent_ind = nearest_search(n, f_pixel(), min_opaque_val, NULL);

    const int max_threads = omp_get_max_threads();
	std::vector<viter_state> average_color(map->colors * max_threads);
...
static void remap_to_palette_floyd(read_info *input_image, write_info *output_image, const colormap *map, const float min_opaque_val, const float *edge_map, const int output_image_is_remapped)
{
	...
	std::vector<float> difference_tolerance(map->colors);
    ...
    for (int row = 0; row < rows; ++row) {
        ...
        do {
			...
			f_pixel err(sa - xp.a, sr - xp.r, sg - xp.g, sb - xp.b);
...
{
    float* noise = (float*) malloc(sizeof(float)*cols*rows);
    float* tmp = (float*) malloc(sizeof(float)*cols*rows);
    float* edges = (float*) malloc(sizeof(float)*cols*rows);
