...
static struct head build_head(f_pixel px, const colormap *map, int num_candidates, mempool *m, std::vector<int>& skip_index, int *skipped)
{
    ...
    for(int i=0; i < num_candidates; i++) {
		...
		h.candidates[i].radius = colors[i].radius;
    }
...
{
    mempool* m = NULL;
    struct nearest_map *centroids = (nearest_map*) mempool_new(m, sizeof(*centroids));
    centroids->mempool = m;

    int skipped=0;
	std::vector<int> skip_index(map->colors);
	for(int j=0; j<map->colors;j++) skip_index[j]=0;
    ...
    for (int i=0; i < map->colors; i++) {
		...
		centroids->heads[h].candidates[centroids->heads[h].num_candidates++].color = map->palette[i].acolor;
		centroids->heads[h].candidates[centroids->heads[h].num_candidates++].index = i;
		centroids->heads[h].candidates[centroids->heads[h].num_candidates++].radius = 999;
    }
