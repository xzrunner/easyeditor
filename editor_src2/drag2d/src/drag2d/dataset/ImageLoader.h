...
{
public:
	static uint8_t* load(const std::string& filepath, int& width, int& height, unsigned int& texture, int& format);
...
private:
	...
	static void loadTexture(unsigned int& texture, uint8_t* pixel, int format, int width, int height, int channels);
	...
	static uint8_t* loadPPM(const std::string& filename, int& width, int& height, int& format);
