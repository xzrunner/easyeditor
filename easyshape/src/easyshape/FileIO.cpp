...
void FileIO::load(const char* filename, LibraryPanel* library)
{
	...
	fin.close();
	{
		...
		ee::LibraryList* list = library->getShapePage()->getList();
	...
	}
	{
		...
		ee::LibraryList* list = library->getImagePage()->getList();
		...
		for (size_t i = 0, n = items.size(); i < n; ++i)
		{
			if (items[i]->getFilepath().empty())
				value["shape"][i] = (dir + "\\" + items[i]->getName() + 
					"_" + ee::FileNameParser::getFileTag(ee::FileNameParser::e_shape) + ".json").ToStdString();
			else
...
LibraryItem* FileIO::loadImageItem(const std::string& filepath)
{
	...
	wxString shape_filepath = ee::FilenameTools::getFilePathExceptExtension(filepath) 
...
void FileIO::store(LibraryItem* item, const std::string& dlg)
{
	...
	wxString filepath;
	wxString ext = "_" + ee::FileNameParser::getFileTag(ee::FileNameParser::e_shape) + ".json";
	if (item->getFilepath().empty())
		filepath = dlg + "\\" + item->getName() + ext;
	else
		filepath = ee::FilenameTools::getFilePathExceptExtension(item->getFilepath()) + ext;
