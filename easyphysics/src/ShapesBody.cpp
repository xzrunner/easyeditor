...
void ShapesBody::getRect(ee::Rect& rect, const ee::Vector& position, float angle) const
{
	rect.MakeInfinite();
	for (b2Fixture* f = m_body->GetFixtureList(); f; f = f->GetNext())
	{
		...
		else if (b2Shape* shape = m_body->GetFixtureList()->GetShape())
		{
			...
			assert(circle);
			rect.Combine(ee::Vector(position.x - circle->m_radius, position.y - circle->m_radius));
			rect.Combine(ee::Vector(position.x + circle->m_radius, position.y + circle->m_radius));
		}
	}
}
