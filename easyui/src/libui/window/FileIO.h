...
public:
	...
	static void FetchSprites(const std::string& filepath, std::vector<ee::SprPtr>& sprs);

private:
	static std::string StoreWrapper(const std::string& filepath,
		const std::string& name, const std::vector<ee::SprPtr>& sprs);

	static void LoadSprites(const Json::Value& val, const Symbol& sym, const std::string& dir);

	static void StoreRefs(Json::Value& val, const Symbol& sym, const std::string& dir);
	static void LoadRefs(const Json::Value& val, Symbol& top_sym, const std::string& dir);
