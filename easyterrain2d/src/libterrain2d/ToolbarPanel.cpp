...
{
	AddChild(new CreateRegionCMPT(this, "创建区域", stage));
	AddChild(m_wave_cmpt = new WaveVerticesCMPT(this, "顶点波动", stage));
	AddChild(m_move_uv_cmpt = new MoveTexcoordsCMPT(this, "纹理移动", stage));
	AddChild(m_blend_tex_cmpt = new BlendTextureCMPT(this, "纹理混合", stage));
	SetSizer(InitLayout());
}
