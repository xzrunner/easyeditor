...
namespace ecomplex
{
		...
		wxWindow*& stage, wxWindow*& toolbar)
	{
		...
		context->property->setPropertySetting(new d2d::SymbolPropertySetting(context->stage, context->stage->getSymbol()));

		context->toolbar = new ToolbarPanel(rightVerticalSplitter, context->stage, context->property);

		toolbar = context->viewlist = new d2d::ViewlistPanel(rightVerticalSplitter, context->stage, context->stage, context->property);
	}
