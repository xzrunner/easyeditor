									...
									const std::vector<int>& ids, const std::vector<std::pair<int, std::string> >& order)
{
	...
	if (dynamic_cast<const d2d::FontBlankSprite*>(sprite))
		resolveSpriteForFrameFont(sprite, cindex);
	else
...
void COCCode::resolveSpriteForFrameFont(const d2d::ISprite* sprite, int id)
{
	std::string assignIndex = lua::assign("index", wxString::FromDouble(id).ToStdString());
