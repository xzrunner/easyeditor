...
{
	return sym->Type() == s2::SYM_ANIM2;
}

void LibraryPage::OnAddPress(wxCommandEvent& event)
{
	std::string ee_filter   = ee::FileHelper::GetJsonFileFilter(ee::SymbolFile::Instance()->Tag(s2::SYM_ANIM2)),
		        json_filter = "JSON files (*.json)|*.json";
	std::string filter = ee_filter + "|" + json_filter;
	wxFileDialog dlg(this, wxT("导入anim2文件"), wxEmptyString, 
...
{
	auto sym = ee::SymbolMgr::Instance()->FetchSymbol(filename, s2::SYM_ANIM2);
	//	sym->RefreshThumbnail(filename);
	m_list->Insert(sym);
