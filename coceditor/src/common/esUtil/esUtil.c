...
GLboolean ESUTIL_API esCreateWindow ( ESContext *esContext, LPCTSTR title, GLint width, GLint height, GLuint flags )
{
   ...
   if(flags & ES_WINDOW_FULLSCREEN) {
	   esContext->width = GetSystemMetrics(SM_CXSCREEN);
		esContext->height = GetSystemMetrics(SM_CYSCREEN);
   } else {
		esContext->width = width;
		esContext->height = height;
   }

   if ( !WinCreate ( esContext, title, flags) )
   {
      return GL_FALSE;
   }
...
void ESUTIL_API esRegisterQuitFunc ( ESContext *esContext, void (ESCALLBACK *quitFunc) (ESContext*) ) {
	esContext->quitFunc = quitFunc;
}
...
void ESUTIL_API esRegisterKeyFunc ( ESContext *esContext,
                                    void (ESCALLBACK *keyFunc) (ESContext*, int, int, int ) )
{
