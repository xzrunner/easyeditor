...
BEGIN_EVENT_TABLE(Frame, wxFrame)
	...
	EVT_MENU(ID_EXTRUDE_IMG, Frame::onExtrudeImg)
...
void Frame::onExtrudeImg(wxCommandEvent& event)
{
	wxFileDialog dlg(this, wxEmptyString, wxEmptyString, wxEmptyString, wxT("*.png"), wxFD_OPEN);
	if (dlg.ShowModal() == wxID_OK)
	{
		std::string path = dlg.GetPath();
		ExtrudeImg extrude(path);
		extrude.Trigger(5);
	}
}
...
wxMenu* Frame::initEditBar()
{
	...
	menu->Append(ID_EXTRUDE_IMG, wxT("ExtrudeImg"), wxT("onExtrudeImg"));
