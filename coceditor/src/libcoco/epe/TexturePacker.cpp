...
void TexturePacker::Pack(const std::set<ee::ImagePtr>& images)
{
	std::vector<ee::ImagePtr> sorted;
...
void TexturePacker::StoreToMemory()
{
	...
	auto itr = m_map_img2rect.begin();
	int size_line = m_edge * channels;
	for ( ; itr != m_map_img2rect.end(); ++itr)
	{
		auto img_data = ee::ImageDataMgr::Instance()->GetItem(itr->first->GetFilepath());
...
void TexturePacker::StoreToFile(const std::string& floder, const std::string& filename)
{
	...
	gimg_export(filepath.c_str(), m_pixels, m_edge, m_edge, GPF_RGBA8, true);
}

const sm::rect* TexturePacker::Query(const ee::ImagePtr& image) const
{
	auto itr = m_map_img2rect.find(image);
