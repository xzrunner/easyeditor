...
void PackPkgMgr::LoadPackage(const Json::Value& val, const std::string& dir)
{
	...
	pkg->path = d2d::FilenameTools::getAbsolutePath(dir, pkg->path);
	d2d::FilenameTools::FormatFilepath(pkg->path);

	pkg->id = val["pkg_id"].asInt();

	std::string ids_file = val["spr_id_file"].asString();
	ids_file = d2d::FilenameTools::getAbsolutePath(dir, ids_file);
	...
	for (int i = 0, n = ids_val.size(); i < n; ++i) {
		...
		filepath = d2d::FilenameTools::getAbsolutePath(pkg->path, filepath);
		d2d::FilenameTools::FormatFilepath(filepath);
