...
void TrailBuilder::Load(const std::shared_ptr<const etrail::Symbol>& sym, PackTrail* trail)
{
	...
	if (trail->mode == T2D_MODE_IMAGE) {
		...
		for (int i = 0; i < cfg->sym_count; ++i) {
			...
			comp.mul_col_begin = gum::color2int(pt2::Color(t_sym.col_begin.r, t_sym.col_begin.g, t_sym.col_begin.b, t_sym.col_begin.a), s2s::ABGR);
			comp.mul_col_end = gum::color2int(pt2::Color(t_sym.col_end.r, t_sym.col_end.g, t_sym.col_end.b, t_sym.col_end.a), s2s::ABGR);
			comp.add_col_begin = gum::color2int(pt2::Color(t_sym.mode.A.add_col_begin.r, t_sym.mode.A.add_col_begin.g, t_sym.mode.A.add_col_begin.b, t_sym.mode.A.add_col_begin.a), s2s::ABGR);
			comp.add_col_end = gum::color2int(pt2::Color(t_sym.mode.A.add_col_end.r, t_sym.mode.A.add_col_end.g, t_sym.mode.A.add_col_end.b, t_sym.mode.A.add_col_end.a), s2s::ABGR);
		...
		for (int i = 0; i < cfg->sym_count; ++i) {
			...
			comp.col_begin = gum::color2int(pt2::Color(t_sym.col_begin.r, t_sym.col_begin.g, t_sym.col_begin.b, t_sym.col_begin.a), s2s::ABGR);
			comp.col_end = gum::color2int(pt2::Color(t_sym.col_end.r, t_sym.col_end.g, t_sym.col_end.b, t_sym.col_end.a), s2s::ABGR);
