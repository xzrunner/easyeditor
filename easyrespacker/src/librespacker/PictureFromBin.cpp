...
void PictureFromBin::Unpack(uint8_t** ptr, const std::vector<ee::ImagePtr>& images,
							PackPicture* pic)
{
	uint16_t sz;
	unpack(sz, ptr);
...
void PictureFromBin::UnpackQuad(uint8_t** ptr, const std::vector<ee::ImagePtr>& images,
								PackPicture::Quad& quad)
{
	uint8_t tex_idx;
	unpack(tex_idx, ptr);
