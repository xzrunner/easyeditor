...
void ShaderMgr::Draw(const vec2 vertices[4], const vec2 texcoords[4], int texid)
{
	SpriteShader* shader = static_cast<SpriteShader*>(m_sprite_shader);
...
void ShaderMgr::SetModelView(const vec2& offset, float scale)
{
...
void ShaderMgr::GetModelView(vec2& offset, float& scale)
{
	offset = m_offset;
	scale = m_scale;
}
