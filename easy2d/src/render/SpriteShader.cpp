...
void SpriteShader::SetTexID(int tex)
{
	...
	{
		if (m_count != 0) {
			log_debug("Shader Commit SetTexture %d to %d", m_tex, tex);
		}
...
void SpriteShader::SetFboID(int fbo)
{
	...
	{
		if (m_count != 0) {
			log_debug("Shader Commit SetFBO %d to %d", m_fbo, fbo);
		}
...
void SpriteShader::Draw(const float vb[16], int texid)
{
	...
	if (++m_count >= MAX_COMMBINE) {
		if (m_count != 0) {
			log_debug("Shader Commit count to max");
		}
		Commit();
	}
}
...
void SpriteShader::LoadShader()
{
	...
	static const char* fs =
		...
		"  if (tmp.r > tmp.g * s && tmp.r > tmp.b * s) { \n"
		"  	  r = tmp.r * v_r_trans; \n"
		"  } \n"
		"  if (tmp.g > tmp.r * s && tmp.g > tmp.b * s) { \n"
		"  	  g = tmp.g * v_g_trans; \n"
		"  } \n"
		"  if (tmp.b > tmp.r * s && tmp.b > tmp.g * s) { \n"
		"    b = tmp.b * v_b_trans; \n"
		"  } \n"
		"  tmp.xyz = (r + g + b).xyz; \n"
