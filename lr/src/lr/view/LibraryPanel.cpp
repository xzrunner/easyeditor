...
void LibraryPanel::InitPages(StagePanel* stage, ee::PropertySettingPanel* property) 
{
	...
	int id = 0;
	{
		LibraryPage* page = new LibraryPage(this, "地形", LT_DEFAULT, id++, ee::CM_PERSPECTIVE_NO_HEIGHT);
	...
	{
		LibraryPage* page = new LibraryPage(this, "装饰", LT_DEFAULT, id++, ee::CM_PERSPECTIVE_AUTO_HEIGHT);
		Layer* layer = page->GetLayer();
		page->AddEditOP(m_stage->GetBaseOP());
		page->AddEditOP(paste_op);
		AddPage(page);
	...
	{
		LibraryPage* page = new LibraryPage(this, "单位", LT_DEFAULT, id++, ee::CM_PERSPECTIVE_AUTO_HEIGHT);
		Layer* layer = page->GetLayer();
		page->AddEditOP(m_stage->GetBaseOP());
		page->AddEditOP(paste_op);
		AddPage(page);
	...
	{
		LibraryPage* page = new LibraryPage(this, "点", LT_DEFAULT, id++, ee::CM_PERSPECTIVE_NO_HEIGHT);
		Layer* layer = page->GetLayer();
		page->AddEditOP(m_stage->GetBaseOP());
		page->AddEditOP(paste_op);
		AddPage(page);
	...
	{
		LibraryPage* page = new LibraryPage(this, "路径", LT_DEFAULT, id++, ee::CM_PERSPECTIVE_NO_HEIGHT);
		Layer* layer = page->GetLayer();
		page->AddEditOP(m_stage->GetBaseOP());
	...
	{
		LibraryPage* page = new LibraryPage(this, "区域", LT_SHAPE, id++, ee::CM_PERSPECTIVE_NO_HEIGHT);
		Layer* layer = page->GetLayer();
		page->AddEditOP(m_stage->GetBaseOP());
	...
	{
		LibraryPage* page = new LibraryPage(this, "碰撞区域", LT_SHAPE, id++, ee::CM_PERSPECTIVE_NO_HEIGHT);
		Layer* layer = page->GetLayer();
		page->AddEditOP(m_stage->GetBaseOP());
		page->AddEditOP(draw_poly_op);
		page->AddEditOP(draw_line_op);
	...
	{
		LibraryPage* page = new LibraryPage(this, "摄像机", LT_DEFAULT, id++, ee::CM_PERSPECTIVE_NO_HEIGHT);
		Layer* layer = page->GetLayer();
		page->AddEditOP(m_stage->GetBaseOP());
		page->AddEditOP(paste_op);
		AddPage(page);
	...
	{
		LibraryPage* page = new LibraryPage(this, "关卡", LT_DEFAULT, id++, ee::CM_PERSPECTIVE_NO_HEIGHT);
		Layer* layer = page->GetLayer();
		page->AddEditOP(m_stage->GetBaseOP());
		page->AddEditOP(paste_op);
		AddPage(page);
...
ee::CameraMode LibraryPanel::GetLayerCameraMode(int idx) const
{
	if (idx < 0 || idx >= m_pages.size()) {
		return ee::CM_ORTHO;
	} else {
