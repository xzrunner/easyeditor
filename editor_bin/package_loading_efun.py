...
EXTRA_OPT = '--opt RGBA8888 --size-constraints POT'
...
pwd = os.getcwd()
build = _mkdir('build_loading_efun')


LUA = _pjoin(pwd, '..', 'tools', 'lua', 'lua52.exe')
EPBIN = _pjoin(pwd, '..', 'tools', 'lua', 'epbin.lua')

res = _abspath(_pjoin(pwd, '..', 'sg_loading_efun'))
png = _pjoin(build, 'loading_efun%d.'+EXT)
tex = _pjoin(build, 'loading_efun%d.json')
tex2 = _pjoin(build, "loading_efun")
loading_lua = _pjoin(build, "loading_efun.lua")
