...
pwd = os.getcwd()
build = _mkdir('build_loading_p91')


LUA = _pjoin(pwd, '..', 'tools', 'lua', 'lua52.exe')
EPBIN = _pjoin(pwd, '..', 'tools', 'lua', 'epbin.lua')

res = _abspath(_pjoin(pwd, '..', 'sg_loading_p91'))
png = _pjoin(build, 'loading_p91%d.'+EXT)
tex = _pjoin(build, 'loading_p91%d.json')
tex2 = _pjoin(build, "loading_p91")
loading_lua = _pjoin(build, "loading_p91.lua")
