...
draw_rect(const struct dfont_rect *rect, int size, struct matrix *mat, uint32_t color, uint32_t additive) {
	...
	shader_draw(vb, color, additive, 0xff0000ff, 0x00ff00ff, 0x0000ffff);
}
...
static void
get_rich_field_color(const struct rich_text *rich, int idx, uint32_t *color, int *offx, int *offy) {
	int i;
	...
	for (i=0;i<rich->count;i++) {
		...
		if (idx >= field->start && idx <= field->end) {
			if (field->type == RL_COLOR) {
				has_color = true;
				fcolor = field->color;
			} else if (field->type == RL_FADE_IN) {
				has_fade_in = true;
				fade_in = field->val;
				*offx = fade_in / 64;
				*offy = fade_in / 32;
			} else if (field->type == RL_HIDE) {
				hide = true;
			}
		}
	}
	if (hide) {
		*color = 0;
		...
		if (has_color) {
			*color = color_mul(fcolor, (*color) | 0xffffff);
		}
		if (has_fade_in) {
			*color = color_mul((*color), (((0xff-fade_in) << 24) | 0xffffff));
		}
	}
}

static bool
get_rich_field_lf(const struct rich_text *rich, int idx, float * offset) {
	int i;
	for (i=0; i<rich->count; i++) {
		struct label_field *field = (struct label_field*)(rich->fields+i);
...
draw_line(const struct rich_text *rich, struct pack_label * l, struct srt *srt, const struct sprite_trans *arg,
	...
	for (j=start; j<end;) {
		...
		if(unicode != '\n') {
			uint32_t field_color = color;
			int offx = 0;
			int offy = 0;
			get_rich_field_color(rich, *pre_char_cnt+char_cnt, &field_color, &offx, &offy);
			cx+=(draw_utf8(unicode, cx+offx, cy+offy, size, srt, field_color, arg, l->edge) + l->space_w)*space_scale;
		}
	}
...
label_size(const char *str, struct pack_label * l, int* width, int* height) {
	...
	max_h += h;
	if (w > max_w) max_w = w;
	if (l->auto_scale > 0 && max_w > l->width) max_w = l->width;
...
label_draw(const struct rich_text *rich, struct pack_label * l, struct srt *srt, const struct sprite_trans *arg) {
	...
	for (i=0; str && str[i];) {
		...
		uint32_t lf = get_rich_field_lf(rich, idx, &space_scale);
