...
renderbuffer_add(struct render_buffer *rb, const struct vertex_pack vb[4], uint32_t color, uint32_t additive,
	...
	for (i=0;i<4;i++) {
		...
		q->p[i].rmap[0] = (rmap >> 24) & 0xff;
		q->p[i].rmap[1] = (rmap >> 16) & 0xff;
		q->p[i].rmap[2] = (rmap >> 8) & 0xff;
		q->p[i].rmap[3] = (rmap) & 0xff;

		q->p[i].gmap[0] = (gmap >> 24) & 0xff;
		q->p[i].gmap[1] = (gmap >> 16) & 0xff;
		q->p[i].gmap[2] = (gmap >> 8) & 0xff;
		q->p[i].gmap[3] = (gmap) & 0xff;

		q->p[i].bmap[0] = (bmap >> 24) & 0xff;
		q->p[i].bmap[1] = (bmap >> 16) & 0xff;
		q->p[i].bmap[2] = (bmap >> 8) & 0xff;
		q->p[i].bmap[3] = (bmap) & 0xff;
	}
	if (++rb->object >= MAX_COMMBINE) {
		return 1;
	}
...
drawquad(struct render_buffer *rb, struct pack_picture *picture, const struct sprite_trans *arg) {
	...
	for (i=0;i<picture->n;i++) {
		...
		if (renderbuffer_add(rb, vb, arg->color, arg->additive, arg->rmap, arg->gmap, arg->bmap)) {
			return 1;
		}
	}
	return 0;
...
polygon_quad(struct render_buffer *rb, const struct vertex_pack *vbp, uint32_t color, uint32_t additive, int max, int index) {
	...
	return renderbuffer_add(rb, vb, color, additive, 0xff0000ff, 0x00ff00ff, 0x0000ffff);
}
