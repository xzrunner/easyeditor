...
#ifdef REPLACE_MEMORY_ALLOCATOR

lua_State *
ejoy2d_lua_init(lua_Alloc f, void *ud) {
...
_alloc(void *ud, void *ptr, size_t osize, size_t nsize) {
	if (nsize == 0) {
		ejoy_free(ud, ptr, osize);
		return NULL;
	}
	else return ejoy_realloc(ud, ptr, osize, nsize);
}
#else
...
#ifdef REPLACE_MEMORY_ALLOCATOR
	G->mem = ejoy_memory_create();
	lua_State *L = ejoy2d_lua_init(_alloc, G->mem);
#else
