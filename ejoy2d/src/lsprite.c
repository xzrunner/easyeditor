...
#define SET_FLAG(u,f,p) do { if(p) { (u)->flags |= (f); } else { (u)->flags &= ~(f); } } while(0)
...
update_p3d_message(struct sprite * s, struct sprite_pack * pack, int parentid, int componentid) {
	struct pack_particle3d* pp3d = (struct pack_particle3d*)pack->data[parentid];
	if (componentid < 0 || componentid >= pp3d->cfg.symbol_count) {
		return;
	}
	s->flags |= SPRFLAG_MESSAGE;
}
...
update_p2d_message(struct sprite * s, struct sprite_pack * pack, int parentid, int componentid) {
	struct pack_particle2d* pp2d = (struct pack_particle2d*)pack->data[parentid];
	if (componentid < 0 || componentid >= pp2d->cfg.symbol_count) {
		return;
	}
	s->flags |= SPRFLAG_MESSAGE;
}
	...
	for (i=0;;i++) {
		...
		if (c) {
			...
			if (s->type == TYPE_ANIMATION) {
				update_anim_message(c, pack, id, i, s->frame);
...
lsetvisible(lua_State *L) {
	...
	SET_FLAG(s, SPRFLAG_INVISIBLE, (b == 0));
	return 0;
}
...
lset_force_inherit_frame(lua_State *L) {
	struct sprite *s = self(L);
	int b = lua_toboolean(L, 2);
	SET_FLAG(s, SPRFLAG_FORCE_INHERIT_FRAME, b);
	return 0;
}
...
lget_random_child_base_frame(lua_State *L) {
	struct sprite *s = self(L);
	lua_pushboolean(L, s->flags & SPRFLAG_RANDOM_CHILD_BASE_FRAME);
	return 1;
}
...
lset_random_child_base_frame(lua_State *L) {
	struct sprite *s = self(L);
	int b = lua_toboolean(L, 2);
	SET_FLAG(s, SPRFLAG_RANDOM_CHILD_BASE_FRAME, b);
	return 0;
}
...
/*  if (lua_isstring(L, 2)) {
	...
	if (!lua_istable(L, 2) || lua_rawlen(L, 2) != 4) {
		return luaL_error(L, "rich text must has a table with 4 items");
	}
...
lanchor_trail(lua_State *L) {
	...
	s->data.anchor->trail = (struct ej_trail_t *)lua_touserdata(L, 2);
	return 0;
}
...
static int
lnewtrail(lua_State *L) {
	struct ej_trail_t *trail = (struct ej_trail_t *)lua_newuserdata(L, sizeof(*trail));
	memset(trail, 0, sizeof(*trail));
	// default value
	trail->seg_count   = TRAIL_DEFAULT_SEG_COUNT;
	trail->update_rate = TRAIL_DEFAULT_UPDATE_RATE;
	trail->extend	   = TRAIL_DEFAULT_EXTEND;
	return 1;
}
...
static struct ej_trail_t*
get_trail(lua_State *L, int idx) {
	struct ej_trail_t *trail = (struct ej_trail_t*)lua_touserdata(L, idx);
	if(!trail) {
		luaL_error(L, "arg 1 must be trail");
		return NULL;
	}
	return trail;
}
...
static int
ltrail_reset(lua_State *L) {
	struct ej_trail_t *trail = get_trail(L, 1);
	trail->inited = 0;
	return 0;
}

static int
ltrail_set_segment(lua_State *L) {
	struct ej_trail_t *trail = get_trail(L, 1);
	trail->seg_count = (uint8_t)luaL_checkinteger(L, 2);
	if(trail->seg_count > TRAIL_MAX_SEG_COUNT) {
		trail->seg_count = TRAIL_MAX_SEG_COUNT;
	}
	return 0;
}

static int
ltrail_set_update_rate(lua_State *L) {
	struct ej_trail_t *trail = get_trail(L, 1);
	trail->update_rate = (uint8_t)luaL_checkinteger(L, 2);
	return 0;
}

static int
ltrail_set_extend(lua_State *L) {
	struct ej_trail_t *trail = get_trail(L, 1);
	trail->extend = (float)luaL_checknumber(L, 2);
	return 0;
}

static int
ltrail_set_color(lua_State *L) {
	struct ej_trail_t *trail = get_trail(L, 1);
	trail->color = luaL_checkinteger(L, 2);
	return 0;
}

static void
ltrail_method(lua_State *L) {
	luaL_Reg l[] = {
		{"reset", ltrail_reset},
		{"set_segment", ltrail_set_segment},
		{"set_update_rate", ltrail_set_update_rate},
		{"set_extend", ltrail_set_extend},
		{"set_color", ltrail_set_color},
		{NULL,NULL},
	};
	luaL_newlib(L, l);
}
