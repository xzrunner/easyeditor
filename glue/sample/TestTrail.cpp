...
void TestTrail::Init()
{
	...
	s2::Symbol* sym = glue::SymbolFactory::Instance()->Create("shape_trail.json");
	{
...
void TestTrail::Resize(int width, int height)
{
	glue::RenderContext::Instance()->OnSize(width, height);
}

void TestTrail::Draw() const
{
