...
{
    const float* c1p = (const float *)&h1p->acolor;
    const float* c2p = (const float *)&h2p->acolor;
...
static f_pixel box_variance(const hist_item achv[], const struct box *box)
{
	...
	return f_pixel(variancea, variancer, varianceg, varianceb);
}
 ...
 */
static int best_splittable_box(const std::vector<box>& bv, int boxes)
{
...
static colormap *colormap_from_boxes(const std::vector<box>& bv,int boxes,hist_item *achv,float min_opaque_val);
static void adjust_histogram(hist_item *achv, const colormap *map, const std::vector<box>& bv, int boxes);
...
static f_pixel averagepixels(int indx, int clrs, const hist_item achv[], float min_opaque_val)
{
	...
	assert(!custom_isnan(r) && !custom_isnan(g) && !custom_isnan(b) && !custom_isnan(a));
