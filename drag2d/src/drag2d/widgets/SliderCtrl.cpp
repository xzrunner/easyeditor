...
SliderCtrl::SliderCtrl(wxPanel* parent, const char* title, const char* name, 
		int default, int min, int max, float scale_slider2text)
	...
	, m_scale_slider2text(scale_slider2text)
{
	...
	m_slider = new wxSlider(this, wxID_ANY, default, min, max, wxDefaultPosition, wxSize(200, -1));
	Connect(m_slider->GetId(), wxEVT_SCROLL_THUMBTRACK, wxScrollEventHandler(SliderCtrl::OnSetValue));
	sizer->Add(m_slider);

	m_text = new wxTextCtrl(this, wxID_ANY, wxString::FromDouble(default), wxDefaultPosition, wxSize(50, -1), wxTE_PROCESS_ENTER);
