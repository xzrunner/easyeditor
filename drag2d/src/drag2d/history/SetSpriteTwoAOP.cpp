...
#define SET_SPRITE_TWO_AOP_DEF(name, var) \
	\
	SetSprite##name##AOP::SetSprite##name##AOP(const std::vector<ISprite*>& sprites, const Vector& val)	\
		: m_new(val) \
	{ \
	...
	\
	Json::Value SetSprite##name##AOP::Store(const std::vector<ISprite*>& sprites) const \
	{ \
		Json::Value ret; \
		return ret; \
	} \
